...
export class AdminOrderRepository {
  ...
  async getOrders(filters: OrderListFilters) {
    ...
    // Transform and filter by amount if needed
    let adminOrders: AdminOrder[] = orders.map((order) => ({
      id: order.id,
      code: order.code,
      userId: order.userId,
    ...
    if (minAmount !== undefined || maxAmount !== undefined) {
      adminOrders = adminOrders.filter((order) => {
        const total = (order.totals as any)?.total || 0;
    ...
    return { orders: adminOrders, total: adminOrders.length };
  }
